(() => {
  const PORTAL_BUILD = "12.0.0-firebase-clean";
...
helpers.showClickEffect = showClickEffect;
  ...
  function bindGlobalEvents() {
    ...
    document.querySelector("#adminManageUsers")?.addEventListener("click", () => {
      ...
      if (!opened) helpers.toast("La gestión de usuarios requiere una cuenta con rol de superadministrador en Firestore.");
    });
  ...
  function loadFirebaseService() {
    ...
    script.onload = () => window.FirebasePortal?.init?.().catch(error => console.warn("[Portal] Firebase no inició.", error));
